...
HELP_DICT = {
    "🧠 **Нейросети (AI):**": {
        "`.ai` / `.аи` _[запрос]_": "Разовый вопрос к Gemini (без памяти).",
        "`.chat` / `.чат` _[текст]_": "Диалог с памятью контекста.",
        "`.text` / `.stt` _(Reply)_": "Распознать ГС/Видео в текст (с эмоциями).",
        "`.ait` _[тема]_": "Написать развернутую статью в Telegraph.",
        "`.chatt` _[текст]_": "Ответ чата в виде статьи Telegraph.",
        "`.model` / `.модель`": "Выбор модели (Flash/Pro).",
        "`.history`": "Показать последние сообщения.",
        "`.reset`": "Сброс памяти диалога."
    },
    "🔊 **Звук (Voice):**": {
        "`.say` / `.скажи` _[текст]_": "Озвучить текст (отправит ГС).",
        "`.saywav` _[текст]_": "Озвучить текст (отправит WAV файл).",
        "`.sayfree` / `.ру` _[текст]_": "Резервная озвучка (FreeTTS).",
        "`.voice` / `.голос`": "Выбор голоса (Puck, Kore...).",
        "`.ttsmodel`": "Выбор движка (Pro/Flash).",
        "`.dialog` / `.t` _[сценарий]_": "Озвучить диалог по ролям.",
        "`.podcast` _[тема]_": "Создать и озвучить подкаст."
    },
    "🎨 Генерация (Image)": {
        "`.img` [промпт]": "Генерация через Google Imagen 3 (High Quality).",
        "`.flux` [промпт]": "Генерация через Flux (Pollinations, Realism)."
    },
    "🛠 **Инструменты:**": {
        "`.stat` / `.стат` _[месяц/год]_": "Аналитика чата (топ слов, мат, люди).",
        "`.cal` / `.кал` _[2+2]_": "Калькулятор.",
        "`.cur` / `.кон` _[100 USD]_": "Конвертер валют.",
        "`.dl` / `.дл` _[ссылка]_": "Скачать видео/музыку.",
        "`.olx` _[запрос]_": "Парсинг OLX в Excel.",
        "`.sys` / `.сис`": "Статус сервера (RPi).",
        "`.s` / `.с` _[текст]_": "Удалить пробелы."
    },
    "🎭 **Настройки:**": {
        "`.sysglobal` _[текст]_": "Глобальная роль для AI.",
        "`.syschat` _[текст]_": "Роль для текущего чата.",
        "`.bl` / `.чс` _(Reply)_": "В черный список.",
        "`.unbl`": "Убрать из ЧС."
    },
    "🎭 Мимикрия (Mimic)": {
        "`.learn` [дней]": "Изучить стиль общения в этом чате и сохранить в базу.",
        "`.mimic` / `.auto`": "Вкл/Выкл режим авто-ответов в текущем чате (Clone Mode)."
    },
    "🤡 **Развлечения:**": {
        "`.sar` _[текст]_": "СдЕлАтЬ сАрКаЗм.",
        "`.spam` _[число]_": "Спам сообщениями.",
